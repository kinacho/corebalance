...
- Los cursos entran en `llms.txt` y en las cards sociales desde `collectCursos()`, que vive en `scripts/lib/cursos.mjs`. ⚠️ Lee los títulos de `CURSOS` con una expresión regular y **comprueba que salen tantos como directorios hay**: si cambia la forma de ese array, el `prebuild` falla en vez de emitir un índice incompleto. Está en su propio módulo y no dentro de `generate-llms.mjs` porque en el `prebuild` el generador de OG corre **antes** que el de llms, así que depender de él sería depender del que va después.
- **Los cursos y las lecciones comparten card social por curso**, generada desde `cursoPages()` en `scripts/lib/og-pages.mjs` con el campo `langs: ['es']` — sin ese campo el bucle escribiría un `-en.png` huérfano por curso. ⚠️ Antes las 34 lecciones caían en el `image` por defecto de `SeoHead`, que **no es una imagen genérica sino la card de la portada en español**, así que compartir una lección enseñaba el titular de la landing. Las lecciones no tienen card propia a propósito: serían 34 PNG y ~4,4 MB para ganar el título exacto. Lo ata `src/lib/seo/og.test.ts`.
