...
{#if showPrompt}
	...
	>
		<button class="absolute top-3 right-3 text-slate-400 hover:text-white" onclick={dismiss} aria-label="Cerrar">✕</button>
		
		<div class="flex items-center gap-3">
			<div class="text-3xl">🚀</div>
			<div>
				<h3 class="font-bold text-white text-sm">Instalar CoreBalance</h3>
				<p class="text-xs text-slate-400 mt-1">Accede más rápido como app nativa.</p>
			</div>
		</div>
