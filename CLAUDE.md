...
- **The maps size themselves through `src/lib/viewport.svelte.ts`, not CSS.** A `viewBox` and an SVG `font-size` are attributes, so a media query cannot touch them; on mobile the canvas is nearly square and the type is much larger, otherwise labels land at 8 px. Everything else about the maps must still be plain CSS — this helper is only for geometry the browser cannot adapt on its own. In the look-through map, mobile cells show **only the percentage** and the names are read from the ranking below, because "Consumo discrecional" truncated to "Consu…" informs nobody.
...
- ⚠️ **`.tab-hidden` flips to `display: block !important` above 1024px** (there are no tabs on desktop). Any container that needs another display mode must re-assert it inside that media query — see `.sidebar-item.tab-hidden { display: flex }` and `.maps-section.tab-hidden { display: grid }`. Forget it and the layout silently collapses to a single column on desktop only.
