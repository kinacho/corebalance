...
- **Deployed-version check**: the landing carries `softwareVersion` in its `SoftwareApplication` schema (from `__APP_VERSION__`, read out of `package.json`), so comparing it against the deployed commit's version is the cheapest way to catch a deploy that never shipped. The CLI retries three times, 15 s apart, because `deployment_status` fires before the CDN has necessarily invalidated the old HTML everywhere — and a guard that goes red at random gets ignored, which is worse than not having it.
- **`.github/workflows/prod-smoke.yml` fires on three triggers and all three are needed.** `deployment_status` (filtered to production successes — Vercel writes that environment name both capitalised and not, so both are accepted) checks each deploy. ⚠️ **The cron every six hours is the one that would actually have caught this incident**, because the DNS broke on its own with no commit involved. And `workflow_dispatch` for after touching DNS or `vercel.json`. No `npm ci` in the job: the script uses only node builtins and `fetch`, so it must be able to run whatever state the repo is in.
- **Known warning, not a bug:** `www.corebalance.app` is NXDOMAIN. The canonical is the apex and `www` appears nowhere in the repo, so the site is fine; anyone typing `www` gets a browser error. It needs a CNAME at the registrar, which is outside this repo — hence a warning that doesn't fail the run.
