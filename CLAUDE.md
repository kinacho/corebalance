...
- **`MapFrame.svelte` is the shared shell of both maps**: header, expand button and expanded view. Expanding **does not mount a second map** — it applies a class to the same container, because a duplicate instance would have its own state and the look-through map's region/sector toggle would disagree between the small and large views. `bind:clientWidth` there is also what makes the type resize when expanding. Note this means jsdom needs a `ResizeObserver`; there is a stub in `src/setupTest.ts`, and tests select a geometry by stubbing `clientWidth` rather than faking `matchMedia`.
