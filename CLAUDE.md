...
- ⚠️ **`CATEGORY_COLORS` cannot be reused inside the deviation map, however tempting.** `CATEGORY_COLORS.core` is *exactly* `DEVIATION_UNDER` and `CATEGORY_COLORS.stocks` is *exactly* `DEVIATION_OVER` — the categories donut and the deviation map already share two hex values with two unrelated meanings, in adjacent carousel lanes. So `BLOCK_HUES` uses cyan for stocks and only reuses violet for satellite. Known, measured, accepted cost: against the blue ramp floor, cyan is ΔE 12.0 normal-vision and violet 6.9 deutan, both under threshold — but that is a failure **between different sections**, which is exactly what faceting addresses (visible gap, block header, per-cell labels) and the remedy the `dataviz` skill prescribes for all-pairs failures. Cyan↔violet, the pair that actually competes, is ΔE 15.0 deutan.
...
- ⚠️ **`text-transform` in CSS after measuring the string in JS is a bug, not a style choice.** The block headers were measured lowercase and rendered uppercase; uppercase is wider, so `approximateTextWidth` under-measured, and the `clipPath` — doing its job as the hard guarantee — cut mid-word: "ACCIONES INDIVIDUALE". The label is now uppercased in the script *before* measuring. General rule: CSS must not change the glyphs after you have measured them.
...
- ⚠️ **The "no target" fill has failed twice, both times by being grey, and the deviation map now admits exactly one grey: the midpoint.** First it was white at 6 %, which over the dark surface is black — in any portfolio where only the core bucket has targets (the demo, and most real ones) six of nine cells vanished and the map looked broken. The fix was a second flat grey, and that failed differently: with six of nine cells grey the whole map read as switched off, and a second neutral also *means* the wrong thing, because it reads as "slightly more on target" when it means **not applicable**. Untargeted cells therefore carry **hue + texture** — a colour from `NO_TARGET_HUES` says which asset, a 45° stripe says it is outside the scale — so the two channels are independent and neither leans on hue discrimination. `NO_TARGET_HUES` deliberately excludes **blue and amber** (they are the two poles, so a blue untargeted cell next to a blue "por debajo" cell is ambiguous) and **pink** (red reads as loss in a portfolio map, and pink beside green is ΔE 1.1 under deuteranopia — a forbidden adjacency of this palette). Within the remaining three, adjacency rules do **not** apply: all three mean the same thing, so confusing them costs nothing. Two consequences that are easy to get wrong: the stripe **darkens** toward `DARK_SURFACE` rather than lightening toward white, because lightening pushed cyan and emerald so high that the white cell labels stopped contrasting; and "sin objetivo" is set apart by *italics*, not by a low alpha — white at 45 % was fine on dark grey and became illegible the moment the cells got a hue. Cells are labelled "sin objetivo" rather than showing a fabricated deviation (`deviation` is `weight − 0`, i.e. the weight again). The diverging mix starts at 60 % toward the pole, not 35 %, so a cell that has just left the band reads as coloured rather than grey. `DeviationTreemap.test.ts` asserts no fill is translucent or near-black **and resolves `url(#…)` to the pattern's own colours** — before that it read a fill it could not parse and silently passed on exactly the cells the check exists for.
...
- ⚠️ **`vercel.json` is schema-validated in strict mode: an unknown top-level key fails the build**, with `should NOT have additional property <key>`. So the usual `"//comment": [...]` trick does **not** work there — it broke a deploy. Any rationale for its contents belongs here instead. It currently holds three things: `headers` for `/.well-known/assetlinks.json` (Android TWA link verification, left over from a parked Play Store deploy) and the web manifest; `github.deploymentEnabled: false`, which stops Vercel from publishing deployments to GitHub because the integration reported the protected immutable deployment URL and left the public repo's "Production" link pointing at a Vercel login (turning off Deployment Protection instead would have made every deployment URL a crawlable duplicate of corebalance.app — the `canonical` is a mitigation, the protection is a barrier); and the `rewrites` entry mapping `/offline` → `/offline.html`, without which the service worker cannot precache its own offline fallback (see the PWA notes above).
